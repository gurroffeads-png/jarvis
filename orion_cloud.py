...
# ======================= CLINICA+ (modulo proprio) =======================
# A logica `cl_*` da Clinica+ foi movida para `clinica_backend.py` (combinado com
# o Gustavo, ver INTEGRAR.md). Os nomes sao injetados neste namespace pelo
# `import clinica_backend` mais abaixo (antes de main()), entao tudo continua
# acessivel como antes (cl_agendar, cl_dashboard, loop_lembretes, pix_brcode, ...).
...
class H(BaseHTTPRequestHandler):
    ...
    def _cl_func(self):
        """Sessao de FUNCIONARIO da clinica (cookie cl_func assinado: cf<uid>:<fid>)."""
        ck = SimpleCookie(self.headers.get("Cookie",""))
        tok = ck["cl_func"].value if "cl_func" in ck else ""
        d = _verify(tok or "")
        if d and d.startswith("cf"):
            try:
                uid, fid = d[2:].split(":"); return {"uid":int(uid), "fid":int(fid)}
            except Exception: return None
        return None
    def _cl_admin(self, uid, cf):
        return bool(cf and next((x for x in cl_func_get(uid) if x.get("id")==cf["fid"] and x.get("admin")), None))
    def _clinica_get(self, path):
        cu = self._user(); cf = self._cl_func()
        uid = cu["id"] if cu else (cf["uid"] if cf else None)
        if uid is None: return self._send({"ok":False,"erro":"faca login"}, 401)
        qs = urllib.parse.parse_qs(self.path.split("?",1)[1]) if "?" in self.path else {}
        g = lambda k, dv=None: (qs.get(k) or [dv])[0]
        if path == "/clinica/estado":
            r = cl_estado(uid); r["sou_dono"] = bool(cu); r["sou_admin"] = bool(cu or self._cl_admin(uid, cf))
            if cf:
                f = next((x for x in cl_func_get(uid) if x.get("id")==cf["fid"]), {})
                r["func"] = {"nome":f.get("nome",""), "admin":bool(f.get("admin"))}
            self._send(r)
        elif path == "/clinica/servicos": self._send({"ok":True, "servicos":cl_servicos_get(uid)})
        elif path == "/clinica/horario":  self._send({"ok":True, "horario":cl_horario_get(uid)})
        elif path == "/clinica/faq":      self._send({"ok":True, "faq":cl_faq_get(uid)})
        elif path == "/clinica/funcionarios":
            self._send({"ok":True, "funcionarios":[cl_func_pub(x) for x in cl_func_get(uid)]} if cu else {"ok":False,"erro":"so o dono"})
        ...
        elif path == "/clinica/relatorios":
            if not cl_pode(uid, "relatorios"): self._send({"ok":False,"upgrade":True,"erro":"Relatorios sao do plano Profissional."})
            else: self._send(cl_relatorios(uid) if (cu or self._cl_admin(uid, cf)) else {"ok":False,"erro":"sem permissao"})
        elif path == "/clinica/caixa":
            self._send(cl_caixa(uid, g("data")) if (cu or self._cl_admin(uid, cf)) else {"ok":False,"erro":"sem permissao"})
        elif path == "/clinica/export": self._send({"ok":True, "rows":cl_export_agendamentos(uid)} if (cu or self._cl_admin(uid, cf)) else {"ok":False,"erro":"sem permissao"})
        else: self._send({"ok":False,"erro":"nao encontrado"}, 404)
    def _clinica_post(self, path, d, u):
        cf = self._cl_func()
        # ---- criar conta da dona (salva de verdade) + login proprio ----
        if path == "/clinica/criar_conta":
            _ip = self._client_ip()
            if not _rate_ok(_ip, "criar", 20, 600): return self._send({"ok":False,"erro":"muitas tentativas, espere um pouco"})
            r = clinica_criar_conta(d)
            if not r.get("ok"): return self._send(r)
            return self._send({"ok":True, "user":_pub(get_user(r["uid"]))}, cookie=make_session(r["uid"]))
        if path == "/clinica/login":
            _ip = self._client_ip()
            if not _rate_ok(_ip, "login"): return self._send({"ok":False,"erro":"muitas tentativas, espere uns minutos"})
            res = clinica_login_dono(d.get("login"), d.get("senha"))
            if res is None: return self._send({"ok":False,"erro":"nao achei essa conta"})
        ...
        if path == "/clinica/func/login":
            ...
            tok = _sign(f"cf{r['uid']}:{r['fid']}")
            https = self.headers.get("X-Forwarded-Proto","").lower()=="https"; sec = "; Secure" if https else ""
            self.send_response(200); self.send_header("Content-Type","application/json; charset=utf-8")
            self.send_header("Set-Cookie", f"cl_func={tok}; Path=/; HttpOnly; SameSite=Lax{sec}; Max-Age=2592000"); self.end_headers()
            self.wfile.write(json.dumps({"ok":True, "funcionario":{"nome":r["nome"], "admin":r["admin"]}}).encode()); return
        if path == "/clinica/func/logout":
            self.send_response(200); self.send_header("Content-Type","application/json")
            self.send_header("Set-Cookie","cl_func=; Path=/; Max-Age=0"); self.end_headers(); self.wfile.write(b'{"ok":true}'); return
        uid = u["id"] if u else (cf["uid"] if cf else None)
        if uid is None: return self._send({"ok":False,"erro":"faca login"}, 401)
        dono = bool(u)   # configuracoes do negocio: so o dono (conta Orion da clinica)
        _rec = {"/clinica/bot":"atendente", "/clinica/promo":"promo", "/clinica/pix/salvar":"pix",
                "/clinica/pacote/salvar":"pacotes", "/clinica/pacote/apagar":"pacotes", "/clinica/pacote/vender":"pacotes",
                "/clinica/estoque/salvar":"estoque", "/clinica/estoque/apagar":"estoque", "/clinica/estoque/mexer":"estoque",
                "/clinica/lembrete/salvar":"lembrete", "/clinica/wa/salvar":"atendente"}.get(path)
        if _rec and not cl_pode(uid, _rec):
            return self._send({"ok":False, "upgrade":True, "erro":"Esse recurso e do plano Profissional. Assine pra liberar."})
        if dono and path == "/clinica/onboarding":     return self._send(cl_onboarding_salvar(uid, d))
        if dono and path == "/clinica/perfil":         return self._send(cl_perfil_set(uid, d))
        if dono and path == "/clinica/servico/salvar": return self._send(cl_servico_salvar(uid, d))
        if dono and path == "/clinica/servico/apagar": return self._send(cl_servico_apagar(uid, d.get("id")))
        if dono and path == "/clinica/horario":        return self._send(cl_horario_set(uid, d))
        if dono and path == "/clinica/func/salvar":    return self._send(cl_func_salvar(uid, d))
        if dono and path == "/clinica/func/apagar":    return self._send(cl_func_apagar(uid, d.get("id")))
        if dono and path == "/clinica/func/ativo":     return self._send(cl_func_ativo(uid, d.get("id"), d.get("ativo")))
        if dono and path == "/clinica/faq":            return self._send(cl_faq_set(uid, d.get("faq")))
        if dono and path == "/clinica/bot":            return self._send({"ok":True, "resposta":cl_bot_agente(uid, d.get("mensagem",""), d.get("historico"), _digs(d.get("tel")) or "5511999990000", d.get("nome") or "Cliente teste")})
        if dono and path == "/clinica/bloqueio/add":   return self._send(cl_bloqueio_add(uid, d))
        if dono and path == "/clinica/bloqueio/remover": return self._send(cl_bloqueio_remover(uid, d.get("id")))
        if dono and path == "/clinica/lembrete/salvar": return self._send(cl_lembrete_set(uid, d))
        ...
        if dono and path == "/clinica/cliente/foto/apagar": return self._send(cl_foto_apagar(uid, d.get("tel"), d.get("id")))
        if dono and path == "/clinica/pacote/salvar":   return self._send(cl_pacote_salvar(uid, d))
        if dono and path == "/clinica/pacote/apagar":   return self._send(cl_pacote_apagar(uid, d.get("id")))
        if dono and path == "/clinica/pacote/vender":   return self._send(cl_pacote_vender(uid, d.get("tel"), d.get("pacote_id"), d.get("pagamento","")))
        if dono and path == "/clinica/estoque/salvar":  return self._send(cl_estoque_salvar(uid, d))
        ...
        self._send({"ok":False,"erro":"sem permissao ou rota invalida"}, 403)
    def _client_ip(self):
    ...
    def _get(self):
        ...
        if mot and not (path in self._GET_PUBLICO or path.startswith("/.well-known") or path.startswith("/pub/")):
        ...
        if path in ("/","/index.html"):
            ...
            self._send(json.dumps({"name":"Clínica+","short_name":"Clínica+","start_url":"/clinica","scope":"/clinica",
                ...
                "icons":[{"src":"/clinica/icon","sizes":"512x512","type":"image/png","purpose":"any maskable"}]}),200,"application/manifest+json")
        elif path == "/clinica/icon": p=_asset("clinica_icon.png","orion_icon.png"); self._file(p,"image/png") if p else self._send(b"",404)
        elif path == "/webhook/whatsapp":   # verificacao do webhook (Meta)
            qs = urllib.parse.parse_qs(self.path.split("?",1)[1]) if "?" in self.path else {}
            g = lambda k: (qs.get(k) or [""])[0]
            verify = gcfg("whatsapp_verify","WHATSAPP_VERIFY_TOKEN") or "orion"
            if g("hub.mode")=="subscribe" and g("hub.verify_token")==verify: self._send(g("hub.challenge"), 200, "text/plain")
            else: self._send("", 403)
        elif path == "/webhook/mp": self._send({"ok":True})   # validacao do Mercado Pago
        elif path.startswith("/pub/"):
            qs = urllib.parse.parse_qs(self.path.split("?",1)[1]) if "?" in self.path else {}
            g = lambda k, dv=None: (qs.get(k) or [dv])[0]
            try: cuid = int(g("c") or 0)
        ...
        elif path == "/admin/clinicas": self._send(admin_clinicas() if (u and u["criador"]) else {"ok":False,"erro":"so o dono"})
    ...
    def _post(self):
        path = self.path
        d = self._body()
        # ---- WEBHOOK do WhatsApp (Meta manda mensagens aqui) ----
        if path == "/webhook/whatsapp":
            try: threading.Thread(target=cl_wa_receber, args=(d,), daemon=True).start()
            except Exception as e: print("[wa hook]", e)
            return self._send({"ok":True})   # responde rapido pro Meta nao reenviar
        if path == "/webhook/mp":   # Mercado Pago avisa pagamento/assinatura
        ...
        if path.startswith("/pub/"):
            _ip = self._client_ip()
            if not _rate_ok(_ip, "pub", 60, 300): return self._send({"ok":False,"erro":"muitas tentativas, tente em instantes"})
            try: cuid = int(d.get("c") or 0)
            except Exception: cuid = 0
            if path == "/pub/agendar":
        ...
        elif path == "/admin/clinica/plano":
            if not (u and u["criador"]): self._send({"ok":False,"erro":"so o dono"}); return
            self._send(cl_plano_set_pro(int(d.get("id")), int(d.get("meses") or 12)) if d.get("pro") else cl_plano_set_free(int(d.get("id"))))
...
import clinica_backend as _clinica_backend  # injeta a logica cl_* da Clinica+ neste modulo
